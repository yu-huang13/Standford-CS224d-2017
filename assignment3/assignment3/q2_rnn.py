...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if length < max_length:
            ...
            ret.append([sentence+[zero_vector]*diff, labels+[zero_label]*diff, [True]*length+[False]*diff])
        else:
...
class RNNModel(NERModel):
    ...
    def create_feed_dict(self, inputs_batch, mask_batch, labels_batch=None, dropout=0.0):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(pretrained_embeddings, [-1, self.max_length, self.config.n_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_drop = tf.nn.dropout(o, 1.0 - dropout_rate)
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        lossMatrix = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)
        loss = tf.reduce_mean(tf.boolean_mask(lossMatrix, self.mask_placeholder))  #return of boolean_mask: 1-D tensor (not 2-D!!!)
        ### END YOUR CODE
        return loss
