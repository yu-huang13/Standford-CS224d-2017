...
feature:
    词hello的feature为[i1, i2], i1为'hello'在word embeding matrix中的index, i2为'CASE:aa'(表示全小写)在word embeding matrix中的index。
    详见data_util.py的featurize函数与ModelHelper.build函数。

fix Bug:
    create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1.0) -> create_feed_dict(self, inputs_batch, labels_batch=None, dropout=0.0),
"""
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        extS = [start] * window_size + sentence + [end] * window_size
        for i in range(window_size, window_size + len(sentence)):
            windowed_data.append(([item for subl in extS[i-window_size: i+window_size+1] for item in subl], labels[i-window_size]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        pretrained_embeddings = tf.nn.embedding_lookup(tf.Variable(self.pretrained_embeddings), self.input_placeholder)
        embeddings = tf.reshape(pretrained_embeddings, [-1, self.config.n_window_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W', [self.config.n_window_features*self.config.embed_size, self.config.hidden_size], initializer=xavier_init)
        b1 = tf.get_variable('b1', self.config.hidden_size, initializer=xavier_init)
        U = tf.get_variable('U', [self.config.hidden_size, self.config.n_classes], initializer=xavier_init)
        b2 = tf.get_variable('b2', self.config.n_classes, initializer=xavier_init)
    ...
    def add_loss_op(self, pred):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred), axis = 0)
        ### END YOUR CODE
        return loss
