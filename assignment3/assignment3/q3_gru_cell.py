...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_r = tf.get_variable('W_r', [self.state_size, self.state_size], initializer=xavier_init)
            U_r = tf.get_variable('U_r', [self.input_size, self.state_size], initializer=xavier_init)
            b_r = tf.get_variable('b_r', self.state_size, initializer=xavier_init)
            W_z = tf.get_variable('W_z', [self.state_size, self.state_size], initializer=xavier_init)
            U_z = tf.get_variable('U_z', [self.input_size, self.state_size], initializer=xavier_init)
            b_z = tf.get_variable('b_z', self.state_size, initializer=xavier_init)
            W_o = tf.get_variable('W_o', [self.state_size, self.state_size], initializer=xavier_init)
            U_o = tf.get_variable('U_o', [self.input_size, self.state_size], initializer=xavier_init)
            b_o = tf.get_variable('b_o', self.state_size, initializer=xavier_init)
