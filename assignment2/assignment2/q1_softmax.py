...
def softmax(x):
    ...
    ### YOUR CODE HERE
    expx = tf.exp(x)        #N * d
    expsum = tf.reduce_sum(expx, axis=1)        #1 * N
    out = expx / tf.reshape(expsum, [tf.shape(expsum)[0], 1])
    #out = expx / tf.reshape(expsum, [expsum.shape[0].value, 1])    #ok too
    ### END YOUR CODE
...
def test_softmax_basic():
    ...
    test3 = softmax(tf.constant(np.array([[1001, 1002, 1003], [3, 4, 5]]), dtype=tf.float32))
    with tf.Session() as sess:
            test3 = sess.run(test3)
    test_all_close("Softmax test 3", test3, np.array([[ 0.09003057,  0.24472847,  0.66524096],
